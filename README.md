...
- **Grounded evidence spans**: `symmemory get --with-evidence` and the `memory_get`/`memory_search` MCP tools can attach the extracted evidence spans that back a stored fact, on demand (default payloads are unchanged).
...
- **Device sync with delete propagation and encrypted relay**: `symmemory sync --remote <url> --token <jwt>` merges two instances bidirectionally (last-writer-wins), propagates deletes via oplog tombstones, and supports `--push-only` / `--pull-only`. With `--relay --passphrase-file <file>` payloads are encrypted client-side (AES-256-GCM); the relay server stores only ciphertext plus minimal metadata (memory id, timestamp).
...
Symaira Memory is in **active pre-1.0 development**. The current release line is `v0.17.x`; configuration and storage formats are stable within a minor version but may evolve between minor releases until the 1.0 stabilization milestone.
