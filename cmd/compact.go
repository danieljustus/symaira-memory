...
engine and then evicting the originals. Use --dry-run to preview without changes.`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		engine := consolidation.NewEngine(database, embeddings, cfg.Consolidation.URL, cfg.Consolidation.Model, cfg.Consolidation.Provider, cfg.Security.PIIEnabled != nil && *cfg.Security.PIIEnabled, cfg.PromptMode, cfg.Consolidation.ParseTimeout())
